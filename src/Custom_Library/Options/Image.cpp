...
{    
    ...
    Image::Image (const SL::Vector2d &shape, const CUST_SL::Color &color): 
    ...
    {
        CUST_SL::Color color = getPixel(SL::Vector2d(uint32_t(x), uint32_t(y)));
        return booba::Color(color.get_r(), color.get_b(), color.get_b(), color.get_a());
    }

    CUST_SL::Color Image::getPixel(SL::Vector2d pos) const
    {
        return getPixel(pos);
    }
